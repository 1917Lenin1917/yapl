...
TEST_CASE("Parsing basic expressions", "[parser]")
{
    ...
    auto plus1_LHS = dynamic_cast<IntegerASTNode*>(minus_LHS->LHS.get());
    auto plus1_RHS = dynamic_cast<BinaryOpASTNode*>(minus_LHS->RHS.get());

    REQUIRE(plus1_LHS->value == 4);

    REQUIRE(plus1_RHS != nullptr);
    REQUIRE(plus1_RHS->op.type == yapl::TOKEN_TYPE::TIMES);
    auto times_LHS = dynamic_cast<IntegerASTNode*>(plus1_RHS->LHS.get());
    auto times_RHS = dynamic_cast<IntegerASTNode*>(plus1_RHS->RHS.get());
    ...
    auto plus2_LHS = dynamic_cast<IntegerASTNode*>(slash_LHS->LHS.get());
    auto plus2_RHS = dynamic_cast<IntegerASTNode*>(slash_LHS->RHS.get());

    REQUIRE(plus2_LHS->value == 1);
    REQUIRE(plus2_RHS->value == 2);
